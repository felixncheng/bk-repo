...
if [[ $ALL -eq 1 || $GATEWAY -eq 1 ]] ; then
    ...
    docker build -f $IMAGE_DIR/gateway/gateway.Dockerfile -t $REGISTRY/bkrepo-gateway:$VERSION $tmp_dir --network=host
    if [[ $PUSH -eq 1 ]] ; then
...
if [[ $ALL -eq 1 || $BACKEND -eq 1 ]] ; then
    ...
    do
        ...
        docker build -f $IMAGE_DIR/backend/backend.Dockerfile -t $REGISTRY/bkrepo-$SERVICE:$VERSION $tmp_dir --network=host
        if [[ $PUSH -eq 1 ]] ; then
...
if [[ $ALL -eq 1 || $INIT -eq 1 ]] ; then
    ...
    docker build -f $IMAGE_DIR/init/init.Dockerfile -t $REGISTRY/bkrepo-init:$VERSION $tmp_dir --no-cache --network=host
    if [[ $PUSH -eq 1 ]] ; then
